import {FieldView, FluidApi, FluidForm, getLabel, PropTypes, React, readOnlyWrapper, Fund} from '../imports';
import {PAGE_NAME} from '../constants';
import {viewTransformer} from './FundTransformer';

export class DropdownFund extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
      ...
      <FieldView>{FluidForm.getValue(this.props.formValue, this.props.field.name, this.transformer)}</FieldView>, (
        <select
          ...
          {this.state.data && this.state.data.map(
            data => (
              <option key={data[Fund.ID]} value={data[Fund.ID]}>{data[Fund.NAME]}</option>)
          )}
        </select>), this.props.readOnly);
  }
}

DropdownFund.propTypes = {
